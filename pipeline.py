...
class Pipeline(ABC):
    def __init__(self, model, output_file, answer_file, count=-1, batch_size=1, attention=False):
        self.model = model
        self.output_file = output_file
        self.answer_file = answer_file
        self.batch_size = batch_size
        self.count = count
        self.store_attention = attention
...
class DirectPipeline(Pipeline):
    ...
    def method(self, data):
        ...
        for i, (prompt, images, true_answers) in enumerate(initial_prompts):
            ...
            outputs = self.model.run_model(prompt, images, self.store_attention)
...
class ZeroShotPipeline(Pipeline):
    ...
    def method(self, data):
        ...
        for i, (prompts, images, true_answers) in enumerate(initial_prompts):
            ...
            outputs = self.model.run_model(final_prompts, images, self.store_attention)
...
class FewShotPipeline(Pipeline):
    ...
    def method(self, data):
        ...
        for i, (prompts, images, true_answers) in enumerate(initial_prompts):
            ...
            print("Running model for final answers")
            outputs = self.model.run_model(final_prompt, images, self.store_attention)
...
class AutoShotPipeline(FewShotPipeline):
    def __init__(self, model, output_file, answer_file, count=-1, batch_size=1, attention=False, example_image=True, k=1):
        super().__init__(model, output_file, answer_file, count, batch_size, attention, example_image, k)
